(() => {
  ...
  function setupImageModal() {
    ...
    modal.innerHTML = `
      <div class="image-modal__panel" role="dialog" aria-modal="true" aria-label="Expanded image view">
        <button type="button" class="image-modal__close" aria-label="Close image viewer">&times;</button>
        <img class="image-modal__img" alt="" />
        <p class="image-modal__caption" hidden></p>
      </div>
    `;
    ...
    if (!(modalImg instanceof HTMLImageElement)) return;
    ...
    function openModal(img) {
      ...
      const customCaption = img.getAttribute("data-caption");
      const captionText = (customCaption || img.alt || "").trim();
      if (captionText) {
